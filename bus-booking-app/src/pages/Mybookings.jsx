      ...
      <div className="mybooking_contiamer">
        {userBookings.length === 0 ? (
          ...
          userBookings.map((booking, index) => {
            ...
            const allowCancel = canCancelBooking(bus.journeyDate, bus.departureTime);
              ...
              <div className="booking-contianer" key={index}>
                <div className="booking-card">
                  ...
                  <p className="booking-info">
                    <span className="booking-label">From:</span> {booking.from} ➡️
                    <span className="booking-label"> To:</span> {booking.to}
                  </p>

                  <p className="booking-info">
                    <span className="booking-label">Journey Date:</span> {bus.journeyDate}
                  </p>

                  <p className="booking-info">
                    <span className="booking-label">Departure:</span> {bus.departureTime}
                  </p>

                   <p className="booking-info">
                    <span className="booking-label">ArrivalTime:</span> {bus.arrivalTime}
                  </p>

                  <p className="booking-info">
                    <span className="booking-label">BookedSeatsNumbers:</span> {booking.seat.join(", ")}
                  </p>

                  <p className="booking-info">
                    <span className="booking-label">Total Fare:</span> ₹{booking.totalFare}
                  </p>
                  ...
                  {allowCancel ? (
                    <button className="cancelBooking" onClick={() => cancelBooking(booking.id)}>
                      Cancel Booking
                    </button>
                  ) : (
                    <p className="cancel-disabled">
                      ⏳ Cancellation not allowed (Bus started or within 5 hours)
                    </p>
                  )}
                </div>
              </div>
